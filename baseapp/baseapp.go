...
func (app *BaseApp) deliverTx(proof *cmtproto.Proof, tx []byte, memTx sdk.Tx, txMultiStore storetypes.MultiStore, txIndex int, incarnationCache map[string]any) *abci.ExecTxResult {
	gInfo := sdk.GasInfo{}
	...
	gInfo, result, anteEvents, err := app.RunTx(proof, execModeFinalize, tx, memTx, txIndex, txMultiStore, incarnationCache)
	if err != nil {
		resultStr = "failed"
...
func (app *BaseApp) RunTx(proof *cmtproto.Proof, mode sdk.ExecMode, txBytes []byte, tx sdk.Tx, txIndex int, txMultiStore storetypes.MultiStore, incarnationCache map[string]any) (gInfo sdk.GasInfo, result *sdk.Result, anteEvents []abci.Event, err error) {
...
func (app *BaseApp) PrepareProposalVerifyTx(tx sdk.Tx) ([]byte, error) {
	...
	_, _, _, err = app.RunTx(nil, execModePrepareProposal, bz, tx, -1, nil, nil)
	if err != nil {
		return nil, err
	}
...
func (app *BaseApp) ProcessProposalVerifyTx(txBz []byte) (sdk.Tx, error) {
	...
	_, _, _, err = app.RunTx(nil, execModeProcessProposal, txBz, tx, -1, nil, nil)
	if err != nil {
		return nil, err
	}
